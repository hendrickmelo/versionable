...
`vX.Y.Z`; C# releases tag `dotnet-vX.Y.Z`. Fully independent streams were rejected because the user-facing compatibility
rule ("match the minor version and files interchange") is worth more than release-cadence freedom; full lockstep
including patch was rejected because early C#-only fixes would spam PyPI with empty releases.
